...
void Tree::radiusSearch(Node* node)
{
  if(node->isLeaf()) 
  {
    if(sphere.intersectsBox(node->box))
    {
      std::shared_ptr<Coords> crd_ptr = _crds.lock();
      for(int i = 0; i < node->nCoords(); i++)
      {
	const XyzVector coordVec = crd_ptr->getVec(node->coordsContained[i]);
	if(sphere.containedInSphere(coordVec.x,coordVec.y,coordVec.z))
	{
	  //std::cout<<coordVec.x << ", " << coordVec.y << ", " << coordVec.z << "\n";
	  //std::cout<<_crds[node->coordsContained[i]] << "\n";
	  nodes++;
	}
      }
    }
